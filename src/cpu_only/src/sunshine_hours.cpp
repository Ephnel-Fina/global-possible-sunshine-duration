...
void SunshineHours::calculate()
{
    switch (calculate_mode)
    {
        case CalculationMode::SINGLE_DEM:
            calculateSunshineHoursCpu(result, target_dem, day_of_year, time_step);
            break;
        case CalculationMode::TARGET_AND_OBSTACLE_DEM:
            throw std::runtime_error("TARGET_AND_OBSTACLE_DEM mode is not implemented in the CPU version");
        default:
            break;
    }
}
